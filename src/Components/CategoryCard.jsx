...
export default function CategoryCard({ category, image, link }) {
  // Configuración de la animación de cada tarjeta individual
  const itemVariants = {
  ...
  return (
    <motion.div
      variants={itemVariants} // Conectamos con la animación del padre
      className="min-w-80 h-80 rounded-2xl relative overflow-hidden shadow-lg cursor-pointer bg-white"
    >
      <Link to={link}>
        {/* Imagen */}
        <img
        ...
        <div className="absolute bottom-0 w-full">
          {/* Onda */}
          <svg
            className="w-full h-8 block" // Agregué 'block' para evitar espacios fantasma
