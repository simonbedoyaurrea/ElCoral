import { Link } from "react-router-dom";

export default function ProductCard({ image,product}) {
  return (
    <div className="cursor-pointer flex flex-col items-center bg-gray-100 rounded-2xl shadow-md overflow-hidden w-full max-w-[180px] sm:max-w-[220px] md:max-w-[350px] transition-transform hover:scale-105">
      <Link to={`/productos/${product.id}`} state={{ product }}>
         
            <div className="w-full aspect-square overflow-hidden max-w-[350px] mx-auto">
              <img
              src={image ?? product.image}
              alt={`Comprar ${product.name} al mejor precio`}
              loading="lazy"
              className="h-full w-full object-cover"
            />
            </div>

              {/* Info */}
              <div className="text-center p-2">
                <h2 className="font-semibold text-gray-800">{product.name}</h2>
                <p className="text-gray-600">${product.price} COP</p>
              </div>
      </Link>
    </div>
  );
} 


