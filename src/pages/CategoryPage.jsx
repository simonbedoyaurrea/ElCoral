...
export default function CategoryPage() {
  ...
  const categoriesMap = {arcillas,aceites,macerados,mantecas,mascarillas,exfoliantes,bronceadores,tratamientos,jabones,balsamos,todos}
  const currentCategory = categoriesMap[categoryName]

  const productsList = (currentCategory && (currentCategory.products || currentCategory.productos)) || [];

  const fuse = useMemo(() => new Fuse(productsList, {
    keys: [
      'name',
      'description'
    ],
  ...
  const filtered = query && query.trim() ? fuse.search(query).map(r => r.item) : productsList;

  useSeo({
    title: currentCategory?.name ? `${currentCategory.name} — EL CORAL` : 'Categoría — EL CORAL',
    description: currentCategory?.description || currentCategory?.descripcion || `Productos de la categoría ${currentCategory?.name || ''}`,
